...
impl FiniteElement {
    ...
    pub fn boundary(&mut self, node_num_glo: usize, dirichlet: Option<f64>, neumann: Option<f64>) {
        if let Some(value) = dirichlet {
            self.vec_b_glo -= value * self.mat_a_glo.row(node_num_glo).to_owned();
    ...
    pub fn solve(&self) -> Array1<f64> {
        self.mat_a_glo.inv().unwrap() * &self.vec_b_glo
    }

    pub fn print_results(&self, unknown_vec_u: &Array1<f64>) {
        println!("Unknown vector u = {:?}", unknown_vec_u);
        println!("Max u = {}, Min u = {}", unknown_vec_u.iter().cloned().fold(f64::NEG_INFINITY, f64::max), unknown_vec_u.iter().cloned().fold(f64::INFINITY, f64::min));
    }
}